...
async function sendCulturedBrain(id, session, team_id) {
    try {
        const res2 = await fetch(`https://www.fibofy.com/panda-sharedchannels/checkUserPrivateChannel/${id}?shared_channel=culturedbrain&team_id=${team_id}&session_type=Nano&bypass=cc4d9d62ad12bb29cce8663cffdaf6e9026961cc81b8dfcf10d4683087403180`, {
            "headers": {
                "accept": "*/*",
                "accept-language": "ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7,uk;q=0.6",
                "sec-ch-ua": "\"Chromium\";v=\"104\", \" Not A;Brand\";v=\"99\", \"Google Chrome\";v=\"104\"",
          ...
          console.log(result1.data.body.channel.id)
          const channel_id = result1.data.body.channel.id
        var response = await fetch(`https://www.fibofy.com/panda-sharedchannels/startNewConversation/${id}/${channel_id}/${session}?shared_channel=culturedbrain&team_id=${team_id}&session_type=Nano`, {
            "headers": {
                "accept": "*/*",
                "authorization": "8a4bfadf32bfb287126fd9ef9ebac259ca32f132bd5cb4f8391edf9a69a06758",
                "accept-language": "ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7,uk;q=0.6",
    ...
    for (let index = 0; index < Data.length; index++) {
        ...
        if ((element.morninghours === date.getHours() && element.morningminutes === date.getMinutes())) {
            ...
            setTimeout(() => {
                ...
                if (element.workspace === CulturedBrainWorkspace) {
                    sendCulturedBrain(element.id, element.morningsession, element.team_id)
                }
        ...
        if ((element.eveninghours === date.getHours() && element.eveningminutes === date.getMinutes())) {
            ...
            setTimeout(() => {
                ...
                if (element.workspace === CulturedBrainWorkspace) {
                    sendCulturedBrain(element.id, element.eveningsession, element.team_id)
                }
                if (element.workspace === PandaWorspace) {
